...
const Profile = () => {
  ...
  useEffect(() => {
    ...
    const fetchProfileAndDependents = async () => {
      try {
        ...
        if (p) {
          ...
          if (p.family_id) {
            const { data: d, error: depErr } = await supabase
              .from("dependents")
              .select("*")
  ...
  const handleAvatar = async (file: File) => {
    ...
    if (file.size > MAX_AVATAR_SIZE_MB * 1024 * 1024) {
      toast({
        ...
        description: `Photo must be under ${MAX_AVATAR_SIZE_MB}MB.`,
        variant: "destructive",
      });
      return;
    }
    ...
    if (profileUpdateErr) {
      toast({ title: "Failed to link photo", description: profileUpdateErr.message, variant: "destructive" });
      return;
    }
  ...
  const addDependent = async () => {
    ...
    if (error) {
      toast({ title: "Could not add dependent", description: error.message, variant: "destructive" });
      return;
    }
  ...
  const removeDependent = async (id: string) => {
    ...
    if (error) {
      toast({ title: "Could not remove dependent", description: error.message, variant: "destructive" });
      return;
    }
      ...
      <div className="space-y-8 max-w-3xl">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid sm:grid-cols-4 gap-2 items-end pt-4 border-t">
              <div className="space-y-2 sm:col-span-2">
                <Label htmlFor="dep_name">Full name</Label>
                <Input
                  id="dep_name"
                  value={newDep.full_name}
                  onChange={(e) => setNewDep({ ...newDep, full_name: e.target.value })}
                  placeholder="e.g. Mary Achieng"
                />
              </div>
              <div className="space-y-2">
                <Label htmlFor="dep_rel">Relationship</Label>
                <Input
                  id="dep_rel"
              ...
              <div className="space-y-2">
                ...
                <Input
                  id="dep_dob"
                  type="date"
                  value={newDep.date_of_birth}
                  onChange={(e) => setNewDep({ ...newDep, date_of_birth: e.target.value })}
                />
              </div>
              <Button
                onClick={addDependent}
                className="w-full sm:col-span-4 sm:w-auto sm:justify-self-start mt-2"
                variant="outline"
              >
